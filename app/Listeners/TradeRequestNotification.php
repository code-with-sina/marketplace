...
class TradeRequestNotification
{
    ...
    public function handle(TradeRequestEvent $event): void
    {
        ...
        Notification::route('sms', trim($user->mobile, '+'))->notify(new Traded(trim($user->mobile, '+'), 'Hi Ratefy User, ' . $recipient->firstname . ' has just placed  trade of  ' . $event->amount . ' units. Powered by Ratefy.'));
